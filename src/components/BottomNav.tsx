...
import { Home, Plus, FolderOpen, MessageCircle, ScanLine, Settings } from 'lucide-react';

const tabs = [
  { path: '/', label: 'الرئيسية', icon: Home },
  { path: '/create', label: 'إنشاء', icon: Plus },
  { path: '/scan', label: 'مسح', icon: ScanLine },
        ...
        {tabs.map(({ path, label, icon: Icon }) => {
          ...
          return (
            <Link
              key={path}
              to={path}
              className={`flex flex-col items-center gap-0.5 px-2 py-1 transition-colors ${active ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`}
            >
